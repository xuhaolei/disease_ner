...
def main():
    ...
    for epoch in range(start_epoch, config.epochs + 1):
        ...
        print("-> Valid time: {:.4f}s, loss = {:.4f}, precision: {:.4f}%, recall: {:.4f}%, F1: {:.4f}%"
              .format(epoch_time, valid_loss, (valid_estimator[0] * 100), (valid_estimator[1] * 100),
                      (valid_estimator[2] * 100)))
        precision.append(valid_estimator[0] * 100)
        recall.append(valid_estimator[1] * 100)
        f1_score.append(valid_estimator[2] * 100)
        ...
        # score
        plt.figure()
        cnt = [i + 1 for i in range(len(f1_score))]
        plt.plot(cnt, precision, "-r")
        plt.plot(cnt, recall, "-b")
        plt.plot(cnt, f1_score, "-g")
        plt.xlabel("epoch")
        plt.ylabel("score")
        ...
        if valid_estimator[0] < best_score:
            ...
            torch.save({"epoch": epoch,
                       ...
                       os.path.join(config.target_dir, "disease_best.pth.tar"))
